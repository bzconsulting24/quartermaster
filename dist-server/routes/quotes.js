...
router.post('/', asyncHandler(async (req, res) => {
    const { number, status, total, currency, issuedAt, expiresAt, notes, accountId, opportunityId, lines } = req.body;
    if (!number || !accountId) {
...
router.post('/:id/convert-to-invoice', asyncHandler(async (req, res) => {
    ...
    const genId = () => `INV-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
    const invoiceId = genId();
    const invoice = await prisma.invoice.create({ data: { id: invoiceId,
            amount,
            status: 'DRAFT',
            issueDate: new Date(),
