...
const summarizeOpportunity = (opportunity) => {
    ...
    if (opportunity.tasks?.length) {
        const tasks = opportunity.tasks
            .slice(0, 2)
            .map((task) => `${task.title} (due ${task.dueDate.toISOString().slice(0, 10)})`)
            .join('; ');
...
const buildResponse = (prompt, opportunitySummary, metrics) => {
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const [opportunities, accounts, contacts, tasks, invoices, quotes, leads, aiInsights] = await Promise.all([
