...
async function enrichAccountData(companyName, notes, owner) {
...
router.post('/', asyncHandler(async (req, res) => {
    const { name, company, email, phone, source, status, owner, score, notes, accountId } = req.body;
    if (!name) {
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    if (lead.account) {
        ...
        const isOrphaned = account.leads.length <= 1 &&
