...
export async function addMessage(threadId, content, fileIds) {
...
export async function executeCRMFunction(name, args) {
    switch (name) {
        ...
        for await (const event of run) {
            if (event.event === 'thread.message.delta') {
                ...
                if (toolOutputs.length > 0) {
                    const submitRun = await openai.beta.threads.runs.submitToolOutputs(event.data.thread_id, event.data.id, { tool_outputs: toolOutputs, stream: true });
