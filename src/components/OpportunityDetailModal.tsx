            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <button onClick={async()=>{ await fetch('/api/activities',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'EMAIL', subject:'Email sent', description:'Outbound email to contact', performedBy:'user', opportunityId: opportunity.id })}); alert('Logged email activity'); }} style={{ padding: '12px', background: 'white', border: '1px solid #E5E7EB', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '500' }}><Mail size={16} color={COLORS.navyDark} /> Send Email
                  </button>
                  <button onClick={async()=>{ await fetch('/api/activities',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'CALL', subject:'Call logged', description:'Call with prospect', performedBy:'user', opportunityId: opportunity.id })}); alert('Logged call'); }} style={{ padding: '12px', background: 'white', border: '1px solid #E5E7EB', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '500' }}><Phone size={16} color={COLORS.navyDark} /> Log Call
                  </button>
                  <button onClick={async()=>{ await fetch('/api/activities',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'MEETING', subject:'Meeting scheduled', description:'Calendar event created', performedBy:'user', opportunityId: opportunity.id })}); alert('Meeting scheduled'); }} style={{ padding: '12px', background: 'white', border: '1px solid #E5E7EB', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '500' }}><Calendar size={16} color={COLORS.navyDark} /> Schedule Meeting
                  </button>
                  <button onClick={async()=>{ const title=prompt('Task title?'); if(!title) return; const due = new Date(Date.now()+86400000).toISOString(); await fetch('/api/tasks',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ title, dueDate: due, assignedTo:'Me', opportunityId: opportunity.id })}); alert('Task created'); }} style={{ padding: '12px', background: 'white', border: '1px solid #E5E7EB', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '500' }}><CheckSquare size={16} color={COLORS.navyDark} /> Create Task
                  </button>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'activity' && (
            <div>
              <div style={{ marginBottom: '16px', display: 'flex', gap: '8px' }}>
                <button onClick={async()=>{ const subject=prompt('Activity subject?'); if(!subject) return; await fetch('/api/activities',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ subject, description:'Quick log', performedBy:'user', opportunityId: opportunity.id })}); }} style={{ padding: '12px', background: 'white', border: '1px solid #E5E7EB', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '500' }}><Plus size={16} /> Log Activity
                </button>
                <button style={{
                  padding: '8px 16px',
                  background: 'white',
