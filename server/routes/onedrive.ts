...
router.get('/auth-url', asyncHandler(async (_req, res) => {
  ...
  if (!clientId || !redirectUri) return res.status(400).json({ message: 'OneDrive not configured' });
...
router.get('/callback', asyncHandler(async (req, res) => {
  ...
  const redirectUri = process.env.MS_REDIRECT_URI || process.env.MS_REDIRECT_URL || process.env.MS_REDIRECT;
  const tenant = process.env.MS_TENANT_ID || 'common';
  if (!code || !clientId || !clientSecret || !redirectUri) return res.status(400).json({ message: 'Missing config/code' });
  ...
  if (!r.ok) return res.status(400).json({ message: 'Token exchange failed' });
...
router.post('/upload/invoice/:id', asyncHandler(async (_req, res) => {
  if (!hasOneDriveAuth()) return res.status(400).json({ message: 'Connect OneDrive first' });
