...
router.get('/auth-url', asyncHandler(async (_req, res) => {
  ...
  if (!url) return res.status(400).json({ message: 'Drive not configured' });
  res.json({ url });
}));
...
async function renderEstimatePdfBuffer(id: number) {
  ...
  if ((quote as any).opportunity?.name) doc.text('Opportunity: ' + (quote as any).opportunity.name);
  doc.moveDown();
  ...
  doc.moveDown(0.2); doc.moveTo(40, doc.y).lineTo(550, doc.y).stroke('#E5E7EB');
  let subtotal = 0;
  doc.fillColor('#111827');
  for (const line of quote.lines) {
    const qty = line.quantity ?? 1; const rate = line.unitPrice; const total = qty * rate; subtotal += total;
    doc.moveDown(0.4);
    doc.text(line.description, 40, undefined, { width: 320 }); const y = doc.y;
  ...
  doc.moveDown(); doc.moveTo(40, doc.y).lineTo(550, doc.y).stroke('#E5E7EB'); doc.moveDown();
...
router.post('/upload/invoice/:id', asyncHandler(async (req, res) => {
  ...
  doc.fillColor('#111827'); doc.moveDown(2); doc.fontSize(18).text(`Invoice #${invoice.id}`);
  let subtotal = 0; for (const it of invoice.items) { subtotal += it.quantity * it.rate; }
...
router.post('/export/estimates/xlsx', asyncHandler(async (_req, res) => {
  ...
  const wb = utils.book_new(); const ws = utils.json_to_sheet(rows as any);
